return (function ()
    ...
    local newstate = function () -- something similar to lua_newstate but for FINAL
        ...
        -- we need somehow to assume in the FLESH.env Tuple<Tuple<Manifest>> in order to work inside artifacts from the get go
        -- so we prepare crucial references for that to work, and then we should just fill up that
        -- I should replace envt with KES
        --local envt = {} -- it's in refactoring in favour for FLESH.KES:resolve("...") 
        ...
        -- meaning of `is` and `=` needs to be refined

        FLESH.KES:write_entry("Native", {
            protocol = {
        ...
        FLESH.KES:write_entry("Number", {
            protocol = {
                responders = {
                    is = {handled = capability_check},
                    ["="] = FLESH.artifact([[function (self, arg)
                        -- create from number literal and native
                        if (FLESH.capcheck(self,arg)) then return arg end -- literal uses same protocol, so we just passing
                        local possibly_number = arg.external
                        if (type(possibly_number) == "number") then
                            return {protocol = self.state,state = {value = arg}}
                        end
            ...
            state = {
                responders = {
                    ["+"] = {handled = FLESH.artifact([[function (self, arg) -- no implicit conversions, this is only between this specific implementation
        ...
        FLESH.KES:write_entry("String", {
            protocol = {
                is = {handled = capability_check},
                ["="] = FLESH.artifact([[function (self, arg)
                    -- same here as number construction
                end]])
            },
            state = {
                responders = {
                ...
                handled = FLESH.artifact([[]]) -- indexing on same level as tuple
            }
        })
        ...
        FLESH.KES:write_entry("Tuple", {
            ...
            state = {
                ...
                handled = FLESH.artifact([[return function (self, arg) -- indexing an python like slicing
                    if arg then -- not nil
                        if (arg) then -- check if it's number artifact and do indexing
                            if (arg.state.value < 0) then 
                                return -- TODO: envt.Error
                            end
                            if (arg.state.value < #self.state.prods) then
                                return self.state.prods[arg.state.value+1]
                            elseif (arg.state.value == #self.state.prods)
                                return self.state.creturn
                            else 
                                return -- TODO: envt.Error
                            end
                        elseif (arg) then -- check if it's a tuple (handle tuple with size 1, as recieving packaged line)
                            if (#arg.state.items < 1) then 
                                return -- TODO: envt.Error
                            end
                        else
                            return -- TODO: envt.Error
        ...
        FLESH.KES:write_entry("Membrane", {
            protocol = {
                is = {handled = capability_check},
                ["="] = {handled = FLESH.artifact([[]])}
            },
            state = {
                unhandled = FLESH.artifact([[return function (self) -- do consider that this is definition of something, meaning it paints
                    ...
                    --if content and content.state then
                        ...
                        elseif (self.state.kind == 1) then -- dynamic
        ...
        FLESH.KES:write_entry("host", { -- I'm probably gonna introduce isolation layer, just to wrap stuff into this.
            protocol = FLESH.KES:resolve("Tuple").state,
            state = {
                labels = FLESH.KES:write_entry(nil,{external = host_enum}),
                data = FLESH.KES:write_entry(nil,{external = {
                    "we probably should put inside artifacts that give interfaces for the types"}})}})
